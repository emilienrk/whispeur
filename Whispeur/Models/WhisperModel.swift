...
// Whispeur
//
// Modèles de données pour les langues supportées par Whisper
// et le catalogue des modèles ggml disponibles au téléchargement.

import Foundation

// MARK: - Langue

/// Représente une langue supportée par le moteur Whisper.
/// La valeur `whisperCode` correspond au code ISO 639-1 attendu par l'API C `whisper_full_params.language`.
/// La valeur spéciale `nil` active la détection automatique de la langue.
struct WhisperLanguage: Identifiable, Hashable, Sendable {
    /// Identifiant unique, ex: "fr", "en", "auto"
    let id: String
    /// Nom affiché dans l'UI
    let displayName: String
    /// Code transmis à `whisper_full_params.language`. `nil` = détection automatique.
    let whisperCode: String?

    // MARK: Mode auto
    static let auto = WhisperLanguage(id: "auto", displayName: "Détection automatique", whisperCode: nil)

    // MARK: Catalogue complet des langues supportées par Whisper
    static let all: [WhisperLanguage] = [
        ...
        WhisperLanguage(id: "bs", displayName: "Bosnien",             whisperCode: "bs"),
    ...
    /// Retourne la langue à partir d'un code ISO, ou `.auto` si introuvable.
...
/// Descripteur d'un modèle Whisper ggml, avec ses métadonnées et son URL de téléchargement HuggingFace.
struct WhisperModelDescriptor: Identifiable, Hashable, Sendable {
    /// Nom technique, ex: "base", "base.en", "small-q5_1"
    let name: String
    /// Taille lisible pour l'UI, ex: "F16 · 142 MiB"
    let sizeInfo: String
    /// URL de téléchargement HuggingFace
    let downloadURL: URL
    /// Nom du fichier ggml, ex: "ggml-base.bin"
    let filename: String
    /// `true` si ce modèle ne supporte que l'anglais
    let isEnglishOnly: Bool

    var id: String { name }

    /// Chemin local du modèle dans Application Support de l'app.
    var localURL: URL {
        let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        return appSupport.appendingPathComponent("Whispeur/Models/\(filename)")
    }

    /// `true` si le fichier modèle est présent sur le disque.
    var isDownloaded: Bool {
        FileManager.default.fileExists(atPath: localURL.path)
    }

    // MARK: Catalogue complet (source : whisper.cpp examples/whisper.swiftui ContentView.swift)
    ...
    static let catalog: [WhisperModelDescriptor] = [
        ...
        // MARK: Large v3 Turbo ✦ recommandé précision/vitesse
    ...
    /// Modèle de développement embarqué : `ggml-base.bin` dans le répertoire `Models/`.
    static var testModel: WhisperModelDescriptor {
        catalog.first { $0.name == "base" }!
    }
}

// MARK: - Initialiseur interne
